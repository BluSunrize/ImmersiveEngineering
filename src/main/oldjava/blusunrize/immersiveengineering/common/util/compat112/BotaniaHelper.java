...
public class BotaniaHelper extends IECompatModule
{
	...
	public void init()
	{
		...
		if(rariryRelic!=null)
		{
			...
			entry.getCase("immersiveengineering:revolver").addLayers(new ShaderLayer(new ResourceLocation("botania:block/livingwood5"), 0xffffffff).setTextureBounds(17/128d, 24/128d, 33/128d, 40/128d));
			entry.getCase("immersiveengineering:drill").addLayers(new ShaderLayer(new ResourceLocation("botania:block/alfheim_portal_swirl"), 0xffffffff).setTextureBounds(14/64d, 10/64d, 26/64d, 22/64d));
			entry.getCase("immersiveengineering:railgun").addLayers(new ShaderLayer(new ResourceLocation("botania:block/storage1"), 0xff9e83eb).setTextureBounds(55/64d, 42/64d, 1, 58/64d).setCutoutBounds(.1875, 0, .75, 1));
			entry.getCase("immersiveengineering:shield").addLayers(new ShaderLayer(new ResourceLocation("botania:block/crate_open"), 0xffffffff).setTextureBounds(0/32f, 9/32f, 14/32f, 26/32f).setCutoutBounds(.0625, 0, .9375, 1));
			...
			if(tag.getBoolean("hardMode"))
				for(ItemEntity item : event.getDrops())
					if(item!=null&&!item.getItem().isEmpty()&&IEContent.itemShaderBag.equals(item.getItem().getItem()))
						ItemNBTHelper.putString(item.getItem(), "rarity", "RELIC");
		}
	}
}