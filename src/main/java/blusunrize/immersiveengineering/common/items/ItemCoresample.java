	...
	public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean adv)
	{
		if(ItemNBTHelper.hasKey(stack, "coords"))
		{
			if(ItemNBTHelper.hasKey(stack, "mineral"))
			{
				...
				String loc = StatCollector.translateToLocal(unloc);
				list.add(StatCollector.translateToLocalFormatted(Lib.CHAT_INFO+"coresample.mineral", (unloc.equals(loc)?mineral:loc)));
			}
			else
				list.add(StatCollector.translateToLocal(Lib.CHAT_INFO+"coresample.noMineral"));
			...
			list.add(s2);
			list.add(StatCollector.translateToLocalFormatted(Lib.CHAT_INFO+"coresample.pos", s0,s1,""));

			if(ItemNBTHelper.hasKey(stack, "infinite"))
				list.add(StatCollector.translateToLocal(Lib.CHAT_INFO+"coresample.infinite"));
			else if(ItemNBTHelper.hasKey(stack, "depletion"))
				list.add(StatCollector.translateToLocalFormatted(Lib.CHAT_INFO+"coresample.yield", ExcavatorHandler.mineralVeinCapacity-ItemNBTHelper.getInt(stack, "depletion")));
			...
			if(ItemNBTHelper.hasKey(stack, "timestamp") && world!=null)
			{
				...
				if(dist<0)
					...
					list.add(StatCollector.translateToLocalFormatted(Lib.CHAT_INFO+"coresample.timestamp", ClientUtils.fomatTimestamp(dist, ClientUtils.TimestampFormat.DHM)));
			}
			else
				list.add(StatCollector.translateToLocal(Lib.CHAT_INFO+"coresample.noTimestamp"));
		}
	}
}