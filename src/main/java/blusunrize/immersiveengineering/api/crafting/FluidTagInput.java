...
{
	protected final Lazy<INamedTag<Fluid>> fluidTag;
	protected final int amount;
	protected final CompoundNBT nbtTag;

	public FluidTagInput(Lazy<INamedTag<Fluid>> fluidTag, int amount, CompoundNBT nbtTag)
	{
	...
	{
		this(Lazy.of(() -> FluidTags.makeWrapperTag(resourceLocation.toString())), amount, nbtTag);
	}
	...
	{
		INamedTag<Fluid> tag = this.fluidTag.get();
		if(tag==null)
	...
	public JsonElement serialize()
	{
		...
		jsonObject.addProperty("tag", this.fluidTag.get().getName().toString());
	...
	{
		out.writeResourceLocation(this.fluidTag.get().getName());
