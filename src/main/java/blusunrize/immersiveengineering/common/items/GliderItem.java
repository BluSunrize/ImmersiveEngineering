		...
		if(!entity.level.isClientSide)
		{
			...
			if(nextFlightTick%10==0)
			{
				...
				if(itemDamage>1 && entity instanceof Player player)
					player.displayClientMessage(new TranslatableComponent(Lib.CHAT_INFO+"glider.too_fast"), true);
				// It also makes worrying noises!
				if(itemDamage>1 && (nextFlightTick+40)%60==0)
					entity.getLevel().playSound(null, entity, IESounds.glider, SoundSource.PLAYERS, 1, 1);
