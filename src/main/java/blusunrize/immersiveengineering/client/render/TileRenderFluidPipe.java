...
public class TileRenderFluidPipe extends TileRenderIE {
  ...
  public void renderStatic(TileEntity tile, Tessellator tes, Matrix4 translationMatrix, Matrix4 rotationMatrix) {
    ...
    if (tile.getWorldObj() == null) {
      ...
      byte connections = 0;
      for (PipeConnection connection : ((TileEntityFluidPipe) tile).connections) {
        if (connection.direction != ForgeDirection.UNKNOWN) {
          ...
          if (connection.type == PipeConnection.Type.TANK) {
            ...
            if (tileEntity instanceof TileEntityFluidPipe) {
              byte cons = ((TileEntityFluidPipe) tileEntity).getConnections();
              if (((TileEntityFluidPipe) tileEntity).connections.size() > 1) {
      ...
      if (((TileEntityFluidPipe) tile).connections.size() == 0) {
        parts.clear();
        conParts.clear();
        parts.add("pipe_y");
        ...
        if (((TileEntityFluidPipe) tile).connections.size() == 1) {
