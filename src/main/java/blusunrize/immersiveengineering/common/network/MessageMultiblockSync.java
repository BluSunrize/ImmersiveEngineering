...
public class MessageMultiblockSync implements IMessage
{
	...
	public void process(Supplier<Context> context)
	{
		context.get().enqueueWork(() -> {
			for(SyncedTemplate synced : templates)
			{
				...
				CompoundNBT sizeSetter = new CompoundNBT();
				ListNBT posList = new ListNBT();
				posList.add(IntNBT.valueOf(synced.size.getX()));
				posList.add(IntNBT.valueOf(synced.size.getY()));
				posList.add(IntNBT.valueOf(synced.size.getZ()));
				sizeSetter.put("size", posList);
				template.read(sizeSetter);
	...
	public static class SyncedTemplate
	{
		...
		private final List<BlockInfo> parts;
