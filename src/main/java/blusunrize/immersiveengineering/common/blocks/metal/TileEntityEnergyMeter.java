...
public class TileEntityEnergyMeter extends TileEntityImmersiveConnectable implements ITickable, IDirectionalTile, IHasDummyBlocks, IAdvancedCollisionBounds,IAdvancedSelectionBounds, IPlayerInteraction, IComparatorOverride
{
	...
	public void placeDummies(BlockPos pos, IBlockState state, EnumFacing side, float hitX, float hitY, float hitZ)
	{
		...
		((TileEntityEnergyMeter)worldObj.getTileEntity(pos.add(0,1,0))).lower = false;
	...
	{
		for(int i=0; i<=1; i++)
			if(worldObj.getTileEntity(getPos().add(0,!lower?-1:0,0).add(0,i,0)) instanceof TileEntityEnergyMeter)
				worldObj.setBlockToAir(getPos().add(0,!lower?-1:0,0).add(0,i,0));
	}
...
//		return new float[]{0,0,0,1,1,1};
	...
	private void updateComparatorValues()
	{
		int oldVal = compVal;
		int maxTrans = 0;
		Set<Connection> conns = ImmersiveNetHandler.INSTANCE.getConnections(worldObj, lower?pos.up():pos);
		if (conns==null)
		{
			compVal = 0;
			return;
