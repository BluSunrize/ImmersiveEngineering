...
public class TileEntitySqueezer extends TileEntityMultiblockMetal<TileEntitySqueezer,SqueezerRecipe> implements IAdvancedSelectionBounds,IAdvancedCollisionBounds, IFluidHandler, IGuiTile
{
	...
	public List<AxisAlignedBB> getAdvancedSelectionBounds()
	{
		...
		if((pos==12||pos==13||pos==15||pos==16)||(pos==21||pos==22||pos==24||pos==25))
		{
			...
			if(pos<18)
				list.add(AxisAlignedBB.fromBounds(0,0,0, 1,.125f,1).offset(getPos().getX(),getPos().getY(),getPos().getZ()));
			if(pos%9>5)
				fl = fl.getOpposite();
			if(pos%3==1)
				fw = fw.getOpposite();
			...
			if(pos>18)
			{
				...
				list.add(AxisAlignedBB.fromBounds(minX,.375f,minZ, maxX,.9375f,maxZ).offset(getPos().getX(),getPos().getY(),getPos().getZ()));
			}
			return list;
		}
		return null;
